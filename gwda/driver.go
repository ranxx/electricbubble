...
import (
	...
	giDevice "github.com/ranxx/electricbubble/gidevice"
...
func NewDriver(capabilities Capabilities, urlPrefix string, mjpegPort ...int) (dsriver WebDriver, err error) {
	if len(mjpegPort) == 0 {
		mjpegPort = []int{9100}
	}
	wd := new(remoteWD)
...
// NewDriver creates new remote client, this will also start a new session.
func NewRemoteDriver(capabilities Capabilities, conns ...net.Conn) (driver WebDriver, err error) {
	if len(conns) == 0 {
		return nil, errors.New("no connection supplied")
	}
	wd := &remoteWD{
		usbCli: &struct {
			httpCli                *http.Client
	...
	wd.usbCli.defaultConn = libimobiledevice.NewGeneralConn(conns[0], 0)
	wd.usbCli.httpCli = convertToHTTPClient(wd.usbCli.defaultConn.RawConn())

	//wd.usbCli.mjpegConn = libimobiledevice.NewGeneralConn(conns[1], 0)
	//wd.mjpegClient = convertToHTTPClient(wd.usbCli.mjpegConn.RawConn())

	dev := Device{
		serialNumber: fmt.Sprintf("%d", rand.Intn(math.MaxInt)),
	}
	...
	slog.Info("转换url成功", "device id", dev.serialNumber)
	_, err = wd.NewSession(capabilities)
	if err != nil {
		return wd, err
	}
	if info, err := wd.DeviceInfo(); err != nil {
		return nil, err
	} else {
		dev.serialNumber = info.UUID
	}

	slog.Info("创建session成功", "device id", dev.serialNumber)
	...
	slog.Info("连接检查线程创建成功", "device id", dev.serialNumber)
...
func NewUSBDriver(capabilities Capabilities, device ...Device) (driver WebDriver, err error) {
	...
	dev := device[0]
	slog.Info("开始连接", "device id", dev.serialNumber)
...
func (wd *remoteWD) Close() error {
	...
	if wd.usbCli.defaultConn != nil {
		// 这行的作用是告诉代理不需要一直尝试连接电脑了，注释掉
		//wd.usbCli.defaultConn.Write([]byte("exit"))
...
func (wd *remoteWD) CloseConn() error {
	if wd == nil || wd.usbCli == nil {
		return nil
	}
	wd.usbCli.Lock()
	defer wd.usbCli.Unlock()
